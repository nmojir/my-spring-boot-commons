...
public class PersianCharNormalizer {
	...
	private static Logger logger = LoggerFactory.getLogger(PersianCharNormalizer.class);
...
//	private static final char YEH_HAMZA_ABOV = '\u0626';
	...
	private boolean enableNormalYEH = false, enableNormalKAF = false, enableNormalHE = false, enableNormalALEF = false,
			enableNormalWAW = false, enableNormalNUMBERS = false;
	...
	/**
	 * Normalize obj fields annotatated with PersianNormalized
	 * @param obj
	 */
		...
		for(Field field: obj.getClass().getDeclaredFields())
		{
			if(field.isAnnotationPresent(PersianNormalized.class)) {
				if(field.getType() != String.class)
					throw new RuntimeException("The field " + field.getName() + " is annotated with @PersianNormalized but is not of type string");
				try {
					logger.trace("Normalizing chars for field " + field.getName());
					field.setAccessible(true);
					field.set(obj, normalize((String)field.get(obj)));
				} catch (IllegalArgumentException | IllegalAccessException e) {
					logger.error("An exception occured while setting field annotated with PersianNormalized. Trace:", e);
				} 
			}
		}
//		return obj;
	...
	private char normalizeYeh(char c) {
		if (c == ARABIC_YEH || c == YEH_BARREE || c == YEH_BARREE_HAMZA_ABOV || c == YEH_MAKSURA
	...
	private char normalizeHe(char c) {
		if (c == HEH_YEH_ABOV || c == HEH_GOAL || c == HEH_GOAL_HAMZA_ABOV || c == HEH_DOCHASHME) {
			c = HEH;
		}
		return c;
	}
	...
	public enum Option {
		...
		/**
		 * convert various type of arabic Yeh to persian Yeh ('ي','ے','ۓ','ى','ئ','ې')
		 * => 'ی'
		 */
		NORMAL_YEH,
		...
		/**
		 * convert arabic and persian numbers to english numbers ('١','٢',...) =>
		 * '1','2',...
		 */
		NORMAL_NUMBERS
	}
